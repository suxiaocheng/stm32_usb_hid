...
/** @defgroup STM3210B_EVAL_I2C_TSENSOR_Private_Function_Prototypes
  * @{
  */
static void LM75_DMA_Config(LM75_DMADirection_TypeDef Direction,
			    uint8_t * buffer, uint8_t NumData);
...
uint16_t LM75_ReadReg(uint8_t RegName)
{
	...
	while (!I2C_CheckEvent
	       (LM75_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) {
		if ((LM75_Timeout--) == 0)
			return LM75_TIMEOUT_UserCallback();
	}
	...
	LM75_Timeout = LM75_FLAG_TIMEOUT;
	while ((!I2C_GetFlagStatus(LM75_I2C, I2C_FLAG_TXE))
	       && (!I2C_GetFlagStatus(LM75_I2C, I2C_FLAG_BTF))) {
		if ((LM75_Timeout--) == 0)
			return LM75_TIMEOUT_UserCallback();
	}
	...
	LM75_Timeout = LM75_FLAG_TIMEOUT;
	while (!I2C_CheckEvent
	       (LM75_I2C, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED)) {
		if ((LM75_Timeout--) == 0)
			return LM75_TIMEOUT_UserCallback();
	}
