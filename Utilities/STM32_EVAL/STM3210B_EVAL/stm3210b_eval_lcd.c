...
void LCD_DrawChar(uint8_t Xpos, uint16_t Ypos, const uint16_t * c)
{
	...
	for (index = 0; index < LCD_Currentfonts->Height; index++) {
		...
		if ((LCDType == LCD_ILI9320) || (LCDType == LCD_SPFD5408)) {
			LCD_CtrlLinesWrite(LCD_NCS_GPIO_PORT, LCD_NCS_PIN,
					   Bit_SET);
		}
		Xaddress++;
		LCD_SetCursor(Xaddress, Ypos);
...
void LCD_FillPolyLine(pPoint Points, uint16_t PointCount)
{
	...
	uint16_t nodes = 0, nodeX[MAX_POLY_CORNERS], pixelX = 0, pixelY = 0, i =
	    0, j = 0, swap = 0;
	uint16_t IMAGE_LEFT = 0, IMAGE_RIGHT = 0, IMAGE_TOP = 0, IMAGE_BOTTOM =
	    0;
...
uint16_t LCD_ReadReg(uint8_t LCD_Reg)
{
	...
	for (i = 0; i < 5; i++) {
		...
		while (SPI_I2S_GetFlagStatus(LCD_SPI, SPI_I2S_FLAG_RXNE) ==
		       RESET) {
		}
...
void LCD_CtrlLinesConfig(void)
{
	...
	/* Enable GPIO clock */
	RCC_APB2PeriphClockCmd(LCD_NCS_GPIO_CLK | LCD_NWR_GPIO_CLK |
			       LCD_RS_GPIO_CLK, ENABLE);
...
void LCD_SPIConfig(void)
{
	...
	/* Enable GPIO clock */
	RCC_APB2PeriphClockCmd(LCD_SPI_SCK_GPIO_CLK | LCD_SPI_MISO_GPIO_CLK |
			       LCD_SPI_MOSI_GPIO_CLK, ENABLE);
