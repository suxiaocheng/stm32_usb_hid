...
SD_Error SD_GetCardInfo(SD_CardInfo * cardinfo)
{
	...
	if ((CardType == SDIO_STD_CAPACITY_SD_CARD_V1_1)
		...
		cardinfo->CardCapacity *=
		    (1 << (cardinfo->SD_csd.DeviceSizeMul + 2));
		...
		cardinfo->CardCapacity =
		    (cardinfo->SD_csd.DeviceSize + 1) * 512 * 1024;
...
SD_Error SD_ReadBlock(uint8_t * readbuff, uint64_t ReadAddr, uint16_t BlockSize)
{
	...
	while (!
	       (SDIO->
		STA & (SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL |
		       ...
		       uint16_t BlockSize)
{
	...
	while (!
	       (SDIO->
		STA & (SDIO_FLAG_DBCKEND | SDIO_FLAG_TXUNDERR |
		       SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT |
		       SDIO_FLAG_STBITERR))) {
			...
			if ((512 - bytestransferred) < 32) {
				restwords =
				    ((512 - bytestransferred) % 4 ==
				     0) ? ((512 -
					    bytestransferred) / 4) : ((512 -
								       bytestransferred)
								      / 4 + 1);
				for (count = 0; count < restwords;
				     count++, tempbuff++, bytestransferred +=
				     4) {
