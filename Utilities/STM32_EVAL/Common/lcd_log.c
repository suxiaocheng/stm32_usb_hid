...
PUTCHAR_PROTOTYPE {
	...
	if (LCD_Lock == DISABLE) {
		...
		if ((LCD_CacheBuffer_xptr < LCD_PIXEL_WIDTH / cFont->Width)
			...
			LCD_CacheBuffer[LCD_CacheBuffer_yptr_bottom].color =
			    LCD_LineColor;
...
static void LCD_LOG_UpdateDisplay(void)
{
	...
	if ((LCD_CacheBuffer_yptr_bottom < (YWINDOW_SIZE - 1)) &&
		...
		LCD_SetTextColor(LCD_CacheBuffer
				 [cnt + LCD_CacheBuffer_yptr_bottom].color);
		LCD_DisplayStringLine((YWINDOW_MIN +
				       LCD_CacheBuffer_yptr_bottom) *
				      cFont->Height,
				      (uint8_t
				       *) (LCD_CacheBuffer[cnt +
		...
		for (cnt = 0; cnt < YWINDOW_SIZE; cnt++) {
			...
			LCD_DisplayStringLine((cnt +
					       YWINDOW_MIN) * cFont->Height,
					      (uint8_t
					       *) (LCD_CacheBuffer[index].
						   line));
...
ErrorStatus LCD_LOG_ScrollBack(void)
{
	...
	if (LCD_ScrollActive == DISABLE) {
		...
		if (LCD_CacheBuffer_yptr_bottom > LCD_CacheBuffer_yptr_top) {
			...
			if ((LCD_CacheBuffer_yptr_bottom -
			     LCD_CacheBuffer_yptr_top) <= YWINDOW_SIZE) {
				LCD_Lock = DISABLE;
				return ERROR;
			}
		}
	...
	if (LCD_ScrollActive == ENABLE) {
		...
		if (LCD_CacheBuffer_yptr_bottom > LCD_CacheBuffer_yptr_top) {
			...
			LCD_CacheBuffer_yptr_bottom--;
		} else if (LCD_CacheBuffer_yptr_bottom <=
			   LCD_CacheBuffer_yptr_top) {
