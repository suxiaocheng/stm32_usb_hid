...
/* No need to redefine __CLZ */
   ...
   */
	...
	__STATIC_INLINE uint32_t arm_recip_q31(q31_t in,
		...
		for (i = 0u; i < 2u; i++) {
			...
			out =
			    (q31_t) clip_q63_to_q31(((q63_t) out * tempVal) >>
						    30u);
		}
   ...
   */
	...
	__STATIC_INLINE q31_t __QADD8(q31_t x, q31_t y) {
		...
		sum =
		    (((q31_t) u << 24) & 0xFF000000) | (((q31_t) t << 16) &
							0x00FF0000) | (((q31_t)
									s << 8)
								       &
								       0x0000FF00)
	...
	__STATIC_INLINE q31_t __QASX(q31_t x, q31_t y) {
		...
		sum =
		    ((sum +
		      clip_q31_to_q15((q31_t) ((short)(x >> 16) + (short)y))) <<
		     16) + clip_q31_to_q15((q31_t) ((short)x -
						    (short)(y >> 16)));
   ...
   */
	void arm_fir_q7(const arm_fir_instance_q7 * S,
			q7_t * pSrc, q7_t * pDst, uint32_t blockSize);
   ...
   */
	void arm_fir_init_q7(arm_fir_instance_q7 * S,
			     uint16_t numTaps,
			     q7_t * pCoeffs, q7_t * pState, uint32_t blockSize);
   ...
   */
	...
	void arm_biquad_cascade_df1_fast_q15(const arm_biquad_casd_df1_inst_q15
					     * S, q15_t * pSrc, q15_t * pDst,
					     uint32_t blockSize);
   ...
   */
	...
	void arm_biquad_cascade_df1_fast_q31(const arm_biquad_casd_df1_inst_q31
					     * S, q31_t * pSrc, q31_t * pDst,
					     uint32_t blockSize);
   ...
   */
	...
	void arm_mat_init_q31(arm_matrix_instance_q31 * S,
			      uint16_t nRows, uint16_t nColumns, q31_t * pData);
   ...
   */
	...
	void arm_mat_init_q15(arm_matrix_instance_q15 * S,
			      uint16_t nRows, uint16_t nColumns, q15_t * pData);
   ...
   */
	...
	void arm_mult_q7(q7_t * pSrcA,
			 q7_t * pSrcB, q7_t * pDst, uint32_t blockSize);
   ...
   */
	...
	void arm_mult_q15(q15_t * pSrcA,
			  q15_t * pSrcB, q15_t * pDst, uint32_t blockSize);
   ...
   */
	...
	void arm_mult_q31(q31_t * pSrcA,
			  q31_t * pSrcB, q31_t * pDst, uint32_t blockSize);
   ...
   */
	...
	void arm_dct4_q31(const arm_dct4_instance_q31 * S,
			  q31_t * pState, q31_t * pInlineBuffer);
   ...
   */
	...
	void arm_scale_q7(q7_t * pSrc,
			  q7_t scaleFract,
			  int8_t shift, q7_t * pDst, uint32_t blockSize);
   ...
   */
	...
	void arm_scale_q31(q31_t * pSrc,
			   q31_t scaleFract,
			   int8_t shift, q31_t * pDst, uint32_t blockSize);
   ...
   */
	...
	void arm_dot_prod_q7(q7_t * pSrcA,
			     q7_t * pSrcB, uint32_t blockSize, q31_t * result);
   ...
   */
	...
	void arm_shift_q7(q7_t * pSrc,
			  int8_t shiftBits, q7_t * pDst, uint32_t blockSize);
   ...
   */
	...
	void arm_shift_q15(q15_t * pSrc,
			   int8_t shiftBits, q15_t * pDst, uint32_t blockSize);
   ...
   */
	...
	void arm_shift_q31(q31_t * pSrc,
			   int8_t shiftBits, q31_t * pDst, uint32_t blockSize);
   ...
   */
	...
	void arm_offset_q7(q7_t * pSrc,
			   q7_t offset, q7_t * pDst, uint32_t blockSize);
   ...
   */
	...
	void arm_offset_q15(q15_t * pSrc,
			    q15_t offset, q15_t * pDst, uint32_t blockSize);
   ...
   */
	...
	void arm_offset_q31(q31_t * pSrc,
			    q31_t offset, q31_t * pDst, uint32_t blockSize);
 ...
 */
   ...
   */
	...
	void arm_conv_q7(q7_t * pSrcA,
			 uint32_t srcALen,
			 q7_t * pSrcB, uint32_t srcBLen, q7_t * pDst);
...
/**
   ...
   */
	...
	arm_status arm_fir_interpolate_init_q31(arm_fir_interpolate_instance_q31
						* S, uint8_t L,
						uint16_t numTaps,
   ...
   */
	...
	arm_status arm_fir_interpolate_init_f32(arm_fir_interpolate_instance_f32
						* S, uint8_t L,
						uint16_t numTaps,
						float32_t * pCoeffs,
						float32_t * pState,
   ...
   */
	...
	void arm_biquad_cas_df1_32x64_q31(const arm_biquad_cas_df1_32x64_ins_q31
					  * S, q31_t * pSrc, q31_t * pDst,
					  uint32_t blockSize);
   ...
   */
	...
	void arm_biquad_cas_df1_32x64_init_q31(arm_biquad_cas_df1_32x64_ins_q31
					       * S, uint8_t numStages,
					       q31_t * pCoeffs, q63_t * pState,
					       uint8_t postShift);
 ...
 /**   
   ...
   */
	...
	void arm_sin_cos_f32(float32_t theta,
			     float32_t * pSinVal, float32_t * pCcosVal);
 ...
 /**
   ...
   */
	...
	__STATIC_INLINE float32_t
	    arm_linear_interp_f32(arm_linear_interp_instance_f32 * S,
				  float32_t x) {
...
//    #if __FPU_USED
   ...
   */
	...
	void arm_min_q7(q7_t * pSrc,
			uint32_t blockSize, q7_t * result, uint32_t * index);
 ...
 */
	...
	void arm_max_q7(q7_t * pSrc,
			uint32_t blockSize, q7_t * pResult, uint32_t * pIndex);
 ...
 */
  ...
  */
	...
	__STATIC_INLINE float32_t arm_bilinear_interp_f32(const
							  arm_bilinear_interp_instance_f32
							  * S, float32_t X,
							  float32_t Y) {
		float32_t out;
  ...
  */
	...
	__STATIC_INLINE q7_t
	    arm_bilinear_interp_q7(arm_bilinear_interp_instance_q7 * S, q31_t X,
				   q31_t Y) {
		q63_t acc = 0;	/* output */
		q31_t out;	/* Temporary output */
		q31_t xfract, yfract;	/* X, Y fractional parts */
