...
/**
  ...
  * @retval None
  */
void RCC_PLLConfig(uint8_t RCC_PLLSource, uint8_t RCC_PLLMul,
		   uint8_t RCC_PLLDiv)
{
	/* Check the parameters */
	...
	*(__IO uint8_t *) CFGR_BYTE3_ADDRESS =
	    (uint8_t) (RCC_PLLSource |
		       ((uint8_t) (RCC_PLLMul | (uint8_t) (RCC_PLLDiv))));
}
...
{
	uint32_t tmp = 0, pllmul = 0, plldiv = 0, pllsource = 0, presc =
	    0, msirange = 0;
	...
	case 0x0C:		/* PLL used as system clock */
		...
		if (pllsource == 0x00) {
			...
			RCC_Clocks->SYSCLK_Frequency =
			    (((HSI_VALUE) * pllmul) / plldiv);
		} else {
			/* HSE selected as PLL clock source */
			RCC_Clocks->SYSCLK_Frequency =
			    (((HSE_VALUE) * pllmul) / plldiv);
		}
		break;
