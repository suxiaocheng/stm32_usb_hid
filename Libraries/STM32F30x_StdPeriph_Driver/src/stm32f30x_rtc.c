		  ...
		  RTC_AlarmTypeDef * RTC_AlarmStruct)
{
	...
	RTC_AlarmStruct->RTC_AlarmMask =
	    (uint32_t) (tmpreg & RTC_AlarmMask_All);
			      ...
			      uint32_t RTC_AlarmSubSecondMask)
{
	...
	tmpreg =
	    (uint32_t) (uint32_t) (RTC_AlarmSubSecondValue) |
	    (uint32_t) (RTC_AlarmSubSecondMask);
				  ...
				  uint32_t RTC_SmouthCalibMinusPulsesValue)
{
	...
	assert_param(IS_RTC_SMOOTH_CALIB_MINUS
	...
	if ((RTC->ISR & RTC_ISR_RECALPF) == RESET) {
		...
		RTC->CALR =
		    (uint32_t) ((uint32_t) RTC_SmoothCalibPeriod | (uint32_t)
...
void RTC_TimeStampCmd(uint32_t RTC_TimeStampEdge, FunctionalState NewState)
{
	...
	tmpreg =
	    (uint32_t) (RTC->CR & (uint32_t) ~ (RTC_CR_TSEDGE | RTC_CR_TSE));
				   ...
				   uint32_t RTC_ShiftSubFS)
{
	...
	if ((RTC->ISR & RTC_ISR_SHPF) == RESET) {
		...
		if ((RTC->CR & RTC_CR_REFCKON) == RESET) {
			...
			RTC->SHIFTR =
			    (uint32_t) (uint32_t) (RTC_ShiftSubFS) |
...
void RTC_ITConfig(uint32_t RTC_IT, FunctionalState NewState)
{
	...
	if (NewState != DISABLE) {
		...
		/* Configure the Interrupts in the RTC_CR register */
		RTC->CR &=
		    (uint32_t) ~ (RTC_IT & (uint32_t) ~ RTC_TAFCR_TAMPIE);
		/* Configure the Tamper Interrupt in the RTC_TAFCR */
		RTC->TAFCR &= (uint32_t) ~ (RTC_IT & RTC_TAFCR_TAMPIE);
	}
	/* Enable the write protection for RTC registers */
