...
void OPAMP_Init(uint32_t OPAMP_Selection, OPAMP_InitTypeDef * OPAMP_InitStruct)
{
	...
	assert_param(IS_OPAMP_INVERTING_INPUT
		     (OPAMP_InitStruct->OPAMP_InvertingInput));
	assert_param(IS_OPAMP_NONINVERTING_INPUT
				    ...
				    OPAMP_InitTypeDef * OPAMP_InitStruct)
{
	...
	/* Check the parameters */
	assert_param(IS_OPAMP_ALL_PERIPH(OPAMP_Selection));
	assert_param(IS_OPAMP_SECONDARY_INVINPUT
		     (OPAMP_InitStruct->OPAMP_InvertingInput));
	assert_param(IS_OPAMP_NONINVERTING_INPUT
		     (OPAMP_InitStruct->OPAMP_NonInvertingInput));
...
uint32_t OPAMP_GetOutputLevel(uint32_t OPAMP_Selection)
{
	...
	if ((*(__IO uint32_t *) (OPAMP_BASE + OPAMP_Selection) &
	     (OPAMP_CSR_OUTCAL)) != 0) {
