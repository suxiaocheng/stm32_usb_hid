...
static const uint16_t TimingTable[5][2] = {	/*       Sector Erase time,            Sector Program time */
...
uint16_t MAL_Init(void)
{
	...
	if ((NOR_ID.Manufacturer_Code == 0x01)
	    && (NOR_ID.Device_Code2 == NOR_S29GL128)) {
		DFU_String_Descriptor[6].Descriptor = DFU_StringInterface2_3;
	}

	/* select the alternate descriptor following NOR ID */
	if ((NOR_ID.Manufacturer_Code == 0x20)
	    && (NOR_ID.Device_Code2 == NOR_M29W128G)) {
...
uint16_t MAL_GetStatus(uint32_t SectorAddress, uint8_t Cmd, uint8_t * buffer)
{
	...
	if ((x == 1) && (NOR_ID.Device_Code2 == NOR_M29W128G)
	    && (NOR_ID.Manufacturer_Code == 0x20)) {
		x = 3;
	} else if ((x == 1) && (NOR_ID.Device_Code2 == NOR_S29GL128)
		   && (NOR_ID.Manufacturer_Code == 0x01)) {
		x = 4;
	}
#endif /* USE_STM3210E_EVAL */
